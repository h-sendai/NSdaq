...
static const char* nsreader_spec[] =
{
    "implementation_id", "NSReader",
    "type_name",         "NSReader",
    "description",       "NSReader component",
    "version",           "1.0",
...
NSReader::NSReader(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("nsreader_out", m_out_data),
      m_sock(0),
      m_recv_byte_size(0),
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("nsreader_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("NSREADER");
}
...
RTC::ReturnCode_t NSReader::onInitialize()
{
    if (m_debug) {
        std::cerr << "NSReader::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t NSReader::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int NSReader::daq_dummy()
{
    return 0;
}

int NSReader::daq_configure()
{
    std::cerr << "*** NSReader::configure" << std::endl;
...
int NSReader::parse_params(::NVList* list)
{
...
int NSReader::daq_unconfigure()
{
    std::cerr << "*** NSReader::unconfigure" << std::endl;

    return 0;
}

int NSReader::daq_start()
{
    std::cerr << "*** NSReader::start" << std::endl;
...
int NSReader::daq_stop()
{
    std::cerr << "*** NSReader::stop" << std::endl;
...
int NSReader::daq_pause()
{
    std::cerr << "*** NSReader::pause" << std::endl;

    return 0;
}

int NSReader::daq_resume()
{
    std::cerr << "*** NSReader::resume" << std::endl;

    return 0;
}

int NSReader::read_data_from_detectors()
{
...
int NSReader::set_data(unsigned int data_byte_size)
{
...
int NSReader::daq_run()
{
    if (m_debug) {
        std::cerr << "*** NSReader::run" << std::endl;
    }
...
extern "C"
{
    ...
    {
        RTC::Properties profile(nsreader_spec);
