...
NSLogger::NSLogger(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_InPort("nslogger_in", m_in_data),
      ...
      m_debug(false)
{
    ...
    registerInPort("nslogger_in", m_InPort);

    init_command_port();
    init_state_table();
    set_comp_name("NSLogger");
}
...
RTC::ReturnCode_t NSLogger::onInitialize()
{
    if (m_debug) {
        std::cerr << "NSLogger::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t NSLogger::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int NSLogger::daq_dummy()
{
    return 0;
}

int NSLogger::daq_configure()
{
    std::cerr << "*** NSLogger::configure" << std::endl;
    int ret = 0;
    m_isDataLogging = false;
...
int NSLogger::parse_params(::NVList* list)
{
    int ret = 0;
    ...
    for (int i = 0; i < length; i += 2) {
        ...
        if (sname == "isLogging") {
            ...
            if (svalue == "yes") {
                ...
                isExistParamLogging = true;
                std::cerr << "NSLogger: Save to file: true\n";
            }
            else if (svalue == "no") {
                m_isDataLogging = false;
                isExistParamLogging = true;
                std::cerr << "NSLogger: Save to file: false\n";
            }
        }
...
int NSLogger::daq_unconfigure()
{
    std::cerr << "*** NSLogger::unconfigure" << std::endl;
    if (m_isDataLogging) {
        delete fileUtils;
        if (m_debug) {
...
int NSLogger::daq_start()
{
    std::cerr << "*** NSLogger::start" << std::endl;
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### ERROR: NSLogger: open file failed\n";
            fatal_error_report(CANNOT_OPEN_FILE);
        }
        else {
            std::cerr << "*** NSLogger: file open succeed\n";
            m_filesOpened = true;
        }
    }
    return 0;
...
int NSLogger::daq_stop()
{
    std::cerr << "*** NSLogger::stop" << std::endl;

    if (m_isDataLogging && m_filesOpened) {
        if (m_debug) {
            std::cerr << "NSLogger::stop: close files \n";
        }
...
int NSLogger::daq_pause()
{
    std::cerr << "*** NSLogger::pause" << std::endl;
    return 0;
}

int NSLogger::daq_resume()
{
    std::cerr << "*** NSLogger::resume" << std::endl;
    return 0;
}

int NSLogger::reset_InPort()
{
    bool ret = true;
    while (ret) {
        ret = m_InPort.read();
    ...
    if (m_debug) {
        std::cerr << "*** NSLogger::InPort flushed\n";
    }
    return 0;
}

void NSLogger::toLower(std::basic_string<char>& s)
{
...
int NSLogger::daq_run()
{
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### NSLogger: ERROR occured at data saving\n";
    ...
    if (m_debug) {
        ...
        if (seq_num % m_update_rate == 0) {
            std::cerr << "NSLogger: loop = " << seq_num << std::endl;
            std::cerr << "\033[A\r";
        }
    }
...
extern "C"
{
    void NSLoggerInit(RTC::Manager* manager)
    {
